...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="Run full GNN pipeline (download, features, graph, train & eval all models)"
    )
    parser.add_argument(
        "--model", type=str,
        choices=["GCN","GAT","GraphSAGE","TemporalGNN"],
        help="Train & evaluate only this specific GNN"
    )
    ...
    if args.all:
        ...
        print(f"4) Training {args.model}…")
        train_gnn(args.model)

    else:
        parser.print_help()
        exit(1)

    print("5) Evaluating trained GNN models…")
    results = evaluate_all()
    print("Pipeline complete. Saved metrics:\n", results)
